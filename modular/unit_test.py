...
def unit_test_news(news_section, news_section_online):
    try:
        script_directory = os.path.dirname(__file__)  # Get the directory of the script
        unit_test_files_directory = os.path.join(script_directory, 'unit_test_fles')
        ...
        for num in simple_items_numbers:
            ...
            if 0 <= numInt < len(news_data):
                ...
                helper.replace_attribute(simple_element, '__image__', 'src', news_data[int(numInt)]['image'])
                helper.replace_attribute(simple_element, '__image__', 'alt', news_data[int(numInt)]['title'])
                helper.replace_attribute(simple_element, '__title__', 'string', news_data[int(numInt)]['title'])
                helper.replace_attribute(simple_element, '__heading__', 'string', news_data[int(numInt)]['heading'])
                helper.replace_attribute(simple_element, '__description__', 'string', news_data[int(numInt)]['description'])
                simple_element = news_section.find(class_="__i_modular_nc_item_" + num)
        ...
        for num in complex_items_numbers:
            ...
            if 0 <= numInt < len(news_data):
                ...
                helper.replace_attribute(complex_element, '__image__', 'src', news_data[int(numInt)]['image'])
                helper.replace_attribute(complex_element, '__image__', 'alt', news_data[int(numInt)]['alt'])
                helper.replace_attribute(complex_element, '__title__', 'string', news_data[int(numInt)]['title'])
                helper.replace_attribute(complex_element, '__heading__', 'string', news_data[int(numInt)]['heading'])
                helper.replace_attribute(complex_element, '__description__', 'string', news_data[int(numInt)]['description'])
                complex_element = news_section.find(class_="__i_modular_c_item_" + num)
        ...
        for num in simple_items_numbers:
            news_section = news_section.replace("__i_modular_nc_item_" + num,"__i_modular_nc_item_" + simple_items_numbers[0])
...
def unit_test_menu(menu_section, menu_section_online):
    try:
        ...
        helper.replace_attribute(menu_section, '__login_register__', 'class','__login_register__2 main-navigation__button2 show-box-login-js button_header logIn d-flex d-lg-none')
        helper.replace_attribute(menu_section, '__login_register__2', 'class','__login_register__ main-navigation__button2 show-box-login-js')
...
def unit_test_footer(footer_section, footer_section_online):
    try:
        ...
        helper.replace_attribute(footer_section, '__aboutUs__', 'string',"تست میکنیم 1 2 3")
        helper.replace_attribute(footer_section, '__address__', 'string',            " آدرس :  تهران، خیابان کارگر شمالی، کوچه چهارم، پلاک ۸ ،ساختمان پلاتین، طبقه سوم، واحدجنوبی")
        helper.replace_attribute(footer_section, '__mobile__', 'string', '''09353834714''')
        helper.replace_attribute(footer_section, '__mobile__', 'href', '''tel:09353834714''')
