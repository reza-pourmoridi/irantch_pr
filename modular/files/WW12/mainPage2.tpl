<!DOCTYPE html>
<html dir="rtl" lang="fa-IR">
 <head>
  <meta charset="UTF-8"/>
  <meta test="i_modular_modulation"/>
  <meta content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0" name="viewport"/>
  <meta content="ie=edge" http-equiv="X-UA-Compatible"/>
  <title>
   آوا پرواز ایرانیان
  </title>
  <link href="project_files/css/bootstrap.min.css" rel="stylesheet"/>
  <link href="project_files/css/owl.carousel.min.css" rel="stylesheet"/>
  <link href="project_files/css/header.css" rel="stylesheet"/>
  <link href="project_files/css/tabs.css" rel="stylesheet"/>
  <link href="project_files/css/select2.css" rel="stylesheet"/>
  <link href="project_files/css/style.css" rel="stylesheet"/>
  <link href="project_files/images/favicon.png" rel="shortcut icon" type="image/x-icon"/>
 </head>
 <body>
  <header class="i_modular_menu header_area">
   <div class="main_header_area">
    <div class="container">
     <nav class="navigation d-flex align-items-center flex-column">
      <div class="d-flex justify-content-between align-items-center w-100 mt-4">
       <div class="nav-header">
        <a class="d-flex" href="__main_link__">
         <img alt="آوا پرواز ایرانیان" src="project_files/images/logo.png"/>
        </a>
       </div>
       <div class="box_button_header">
        <html>
         <body>
          <a class="__login_register_class__ main-navigation__button2 show-box-login-js button_header logIn d-flex d-lg-none" href="javascript:">
           <i>
            <svg viewbox="0 0 448 512">
             <path d="M224 256c70.7 0 128-57.31 128-128s-57.3-128-128-128C153.3 0 96 57.31 96 128S153.3 256 224 256zM224 32c52.94 0 96 43.06 96 96c0 52.93-43.06 96-96 96S128 180.9 128 128C128 75.06 171.1 32 224 32zM274.7 304H173.3C77.61 304 0 381.6 0 477.3c0 19.14 15.52 34.67 34.66 34.67h378.7C432.5 512 448 496.5 448 477.3C448 381.6 370.4 304 274.7 304zM413.3 480H34.66C33.2 480 32 478.8 32 477.3C32 399.4 95.4 336 173.3 336h101.3C352.6 336 416 399.4 416 477.3C416 478.8 414.8 480 413.3 480z">
             </path>
            </svg>
           </i>
           <span>
            {include file="`$smarty.const.FRONT_CURRENT_THEME`topBarName.tpl"}
           </span>
          </a>
          <div class="main-navigation__sub-menu2 arrow-up show-content-box-login-js" style="display: none">
           {include file="`$smarty.const.FRONT_CURRENT_THEME`topBar.tpl"}
          </div>
         </body>
        </html>
        <a class="button_header_phone d-none d-lg-flex" href="javascript:">
         <span>
          09120517308
         </span>
         <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
          <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
          <path d="M342.5 303.6l-54.24-23.25c-6.391-2.766-13.9-.9062-18.24 4.484l-22.02 26.91c-34.63-17-62.77-45.14-79.77-79.75l26.91-22.05c5.375-4.391 7.211-11.83 4.492-18.22L176.4 137.5C173.4 130.5 165.8 126.7 158.5 128.4L108.1 140c-7.125 1.625-12.11 7.891-12.11 15.22c0 126.1 102.6 228.8 228.7 228.8c7.336 0 13.6-4.984 15.24-12.11l11.62-50.39C353.3 314.2 349.5 306.6 342.5 303.6zM384 32H64C28.65 32 0 60.66 0 96v320c0 35.34 28.65 64 64 64h320c35.35 0 64-28.66 64-64V96C448 60.66 419.3 32 384 32zM400 416c0 8.82-7.178 16-16 16H64c-8.822 0-16-7.18-16-16V96c0-8.82 7.178-16 16-16h320c8.822 0 16 7.18 16 16V416z">
          </path>
         </svg>
        </a>
       </div>
       <div class="nav-toggle mr-3">
        <svg viewbox="0 0 448 512">
         <path d="M0 80C0 71.16 7.164 64 16 64H432C440.8 64 448 71.16 448 80C448 88.84 440.8 96 432 96H16C7.164 96 0 88.84 0 80zM0 240C0 231.2 7.164 224 16 224H432C440.8 224 448 231.2 448 240C448 248.8 440.8 256 432 256H16C7.164 256 0 248.8 0 240zM432 416H16C7.164 416 0 408.8 0 400C0 391.2 7.164 384 16 384H432C440.8 384 448 391.2 448 400C448 408.8 440.8 416 432 416z">
         </path>
        </svg>
       </div>
      </div>
      <div class="nav-menus-wrapper">
       <ul class="nav-menu">
        <li>
         <a href="{$smarty.const.ROOT_ADDRESS}/page/flight">
          پرواز
         </a>
        </li>
        <li>
         <a href="{$smarty.const.ROOT_ADDRESS}/mag">
          وبلاگ
         </a>
        </li>
        <li>
         <a href="{$smarty.const.ROOT_ADDRESS}/news">
          اخبار سایت
         </a>
        </li>
        <li>
         <a href="{$smarty.const.ROOT_ADDRESS}/aboutIran">
          معرفی ايران
         </a>
        </li>
        <li>
         <a href="javascript:">
          آژانس ما
         </a>
         <ul class="nav-dropdown">
          <li>
           <a href="{$smarty.const.ROOT_ADDRESS}/UserTracking">
            پیگیری خرید
           </a>
          </li>
          <li>
           <a href="{$smarty.const.ROOT_ADDRESS}/rules">
            قوانین و مقررات
           </a>
          </li>
          <li>
           <a href="{$smarty.const.ROOT_ADDRESS}/aboutUs">
            درباره ما
           </a>
          </li>
          <li>
           <a href="{$smarty.const.ROOT_ADDRESS}/contactUs">
            تماس با ما
           </a>
          </li>
          <li>
           <a href="javascript:">
            فایل های راهنما
           </a>
          </li>
         </ul>
        </li>
        <li class="d-none d-lg-flex">
         <a class="__login_register_class__2 main-navigation__button2 show-box-login-js" href="javascript:" st="">
          {include file="`$smarty.const.FRONT_CURRENT_THEME`topBarName.tpl"}
         </a>
        </li>
       </ul>
      </div>
     </nav>
    </div>
   </div>
  </header>
  <main>
   <section class="banner">
    <div class="i_modular_banner_gallery OWL_slider_banner owl-carousel owl-theme">
     {foreach $banners as $key =&gt; $banner}
     <div class="__i_modular_nc_item_class_0 item">
      <img alt='{$banner["title"]}' class="__image_class__" src='{$banner["pic"]}'/>
     </div>
     {/foreach}
    </div>
    <div class="context_banner">
     <div class="textBox_banner">
      <div class="container">
       <span>
        آوا پرواز ایرانیان
       </span>
       <h2>
        بلیط هواپیمای داخلی و خارجی
       </h2>
      </div>
     </div>
     <div class="searchBox_banner">
      <div class="container">
       <div class="searchs_box">
        <ul class="nav nav-tabs" id="myTab" role="tablist" style="display: none">
         <li class="nav-item">
          <a aria-controls="flight-internal" aria-selected="true" class="nav-link active" data-toggle="tab" href="#flight-internal" id="flight-internal-tab" role="tab">
           <div>
            <svg viewbox="0 0 576 512" xmlns="http://www.w3.org/2000/svg">
             <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
             <path d="M576 256C576 305 502.1 336 464.2 336H382.2L282.4 496C276.4 506 266.4 512 254.4 512H189.5C179.5 512 169.5 508 163.5 500C157.6 492 155.6 480.1 158.6 471L201.5 336H152.5L113.6 388C107.6 396 98.61 400 88.62 400H31.7C22.72 400 12.73 396 6.74 388C.7485 380-1.248 370 1.747 360L31.7 256L.7488 152C-1.248 143 .7488 133 6.74 125C12.73 117 22.72 112 31.7 112H88.62C98.61 112 107.6 117 113.6 125L152.5 176H201.5L158.6 41C155.6 32 157.6 21 163.5 13C169.5 5 179.5 0 189.5 0H254.4C265.4 0 277.4 7 281.4 16L381.2 176H463.2C502.1 176 576 208 576 256H576zM527.1 256C525.1 246 489.1 224 463.2 224H355.3L245.4 48H211.5L266.4 224H128.6L80.63 160H53.67L81.63 256L53.67 352H80.63L128.6 288H266.4L211.5 464H245.4L355.3 288H463.2C490.1 288 526.1 267 527.1 256V256z">
             </path>
            </svg>
            <h4>
             پرواز داخلی
            </h4>
           </div>
          </a>
         </li>
         <li class="nav-item">
          <a aria-controls="flight-international" aria-selected="false" class="nav-link" data-toggle="tab" href="#flight-international" id="flight-international-tab" role="tab">
           <div>
            <svg viewbox="0 0 576 512" xmlns="http://www.w3.org/2000/svg">
             <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
             <path d="M576 256C576 305 502.1 336 464.2 336H382.2L282.4 496C276.4 506 266.4 512 254.4 512H189.5C179.5 512 169.5 508 163.5 500C157.6 492 155.6 480.1 158.6 471L201.5 336H152.5L113.6 388C107.6 396 98.61 400 88.62 400H31.7C22.72 400 12.73 396 6.74 388C.7485 380-1.248 370 1.747 360L31.7 256L.7488 152C-1.248 143 .7488 133 6.74 125C12.73 117 22.72 112 31.7 112H88.62C98.61 112 107.6 117 113.6 125L152.5 176H201.5L158.6 41C155.6 32 157.6 21 163.5 13C169.5 5 179.5 0 189.5 0H254.4C265.4 0 277.4 7 281.4 16L381.2 176H463.2C502.1 176 576 208 576 256H576zM527.1 256C525.1 246 489.1 224 463.2 224H355.3L245.4 48H211.5L266.4 224H128.6L80.63 160H53.67L81.63 256L53.67 352H80.63L128.6 288H266.4L211.5 464H245.4L355.3 288H463.2C490.1 288 526.1 267 527.1 256V256z">
             </path>
            </svg>
            <h4>
             پرواز خارجی
            </h4>
           </div>
          </a>
         </li>
        </ul>
        <div class="tab-content" id="myTabContent">
         <div aria-labelledby="flight-international-tab" class="tab-pane active show" id="flight-international" role="tabpanel">
          <div>
           <form class="d_contents" id="gds_portal" method="post" name="gds_portal" target="_blank">
            <div class="col-md-12 col-xs-12 col-sm-12 d-ceckboxs">
             <div class="cntr">
              <label class="btn-radio" for="rdo-3">
               <input checked="" class="multiselectportal" id="rdo-3" name="select-rb" type="radio" value="1"/>
               <span>
                یک طرفه
               </span>
              </label>
              <label class="btn-radio" for="rdo-4">
               <input class="multiselectportal" id="rdo-4" name="select-rb" type="radio" value="2"/>
               <span>
                دو طرفه
               </span>
              </label>
             </div>
            </div>
            <div class="col-lg-2 col-md-6 col-sm-12 col-12 col_search search_col col_with_route">
             <div class="form-group origin_start">
              <input class="form-control inputSearchForeign" id="OriginPortal" name="OriginPortal" placeholder="مبدأ ( نام شهر یا فرودگاه )" type="text"/>
              <img alt="loader" class="loaderSearch" id="loaderSearch" name="loaderSearch" src="project_files/images/loader.gif" style="display: none"/>
              <input class="" id="OriginAirportPortal" name="OriginAirportPortal" type="hidden" value=""/>
              <div class="resultUlInputSearch" id="ListAirPort" style="display: none">
              </div>
              <div class="ListAirPort" id="ListAirPort_2">
               <div class="first">
                <h4 class="m-0">
                 <span class="c-text">
                  <div class="c-text2">
                  </div>
                  <div class="yata_gdemo">
                   <i>
                   </i>
                   <em>
                   </em>
                  </div>
                 </span>
                </h4>
               </div>
              </div>
             </div>
             <button class="switch_routs" name="button" onclick="reversDestination('internationalFlights')" type="button">
              <svg viewbox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
               <path d="M488 344H79.24l74.29-79.63C162.6 254.7 162.1 239.5 152.4 230.5C142.7 221.4 127.5 221.9 118.5 231.6l-112 120c-8.625 9.219-8.625 23.53 0 32.75l112 120C123.2 509.4 129.6 512 136 512c5.875 0 11.75-2.125 16.38-6.469c9.688-9.031 10.22-24.22 1.156-33.91L79.24 392H488c13.25 0 24-10.75 24-24S501.3 344 488 344zM24 168h408.8l-74.29 79.63c-9.062 9.688-8.531 24.88 1.156 33.91c9.656 9.094 24.88 8.562 33.91-1.156l112-120c8.625-9.219 8.625-23.53 0-32.75l-112-120C388.8 2.562 382.4 0 376 0c-5.875 0-11.75 2.125-16.38 6.469c-9.688 9.031-10.22 24.22-1.156 33.91L432.8 120H24C10.75 120 0 130.8 0 144S10.75 168 24 168z">
               </path>
              </svg>
             </button>
            </div>
            <div class="col-lg-2 col-md-6 col-sm-12 col-12 col_search search_col">
             <div class="form-group">
              <span class="destnition_start">
               <input class="inputSearchForeign form-control" id="DestinationPortal" name="DestinationPortal" placeholder="مقصد ( نام شهر یا فرودگاه )" type="text"/>
              </span>
              <input class="" id="DestinationAirportPortal" name="DestinationAirportPortal" type="hidden" value=""/>
              <div class="resultUlInputSearch" id="ListAirPortDestination" style="display: none">
              </div>
              <div class="ListAirPort" id="ListAirPortDestination_2">
               <div class="first">
                <h4 class="m-0">
                 <span class="c-text">
                  <div class="c-text2">
                  </div>
                  <div class="yata_gdemo">
                   <i>
                   </i>
                   <em>
                   </em>
                  </div>
                 </span>
                </h4>
               </div>
              </div>
             </div>
            </div>
            <div class="col-lg-4 col-md-12 col-sm-12 col-12 col_search date_search">
             <div class="form-group">
              <input class="deptCalendar form-control went" id="gds_dept_date_Portal" name="gds_dept_date_portal" placeholder="تاریخ رفت" readonly="" type="text"/>
             </div>
             <div class="form-group">
              <input class="form-control return_input2 checktest1 returnCalendar" disabled="" id="regds_dept_date_Portal" name="regds_dept_date_Portal" placeholder="تاریخ برگشت" readonly="" type="text"/>
             </div>
            </div>
            <div class="col-lg-2 col-md-6 col-sm-12 col-12 col_search">
             <div class="select inp-s-num adt box-of-count-nafar">
              <input class="l-bozorgsal-2" id="qtyadt" name="gds_infants_no_portal" type="hidden" value="1"/>
              <input class="l-kodak-2" id="qtychd" name="gds_childs_no_portal" type="hidden"/>
              <input class="l-nozad-2" id="qtyinf" name="gds_infants_no_portal" type="hidden"/>
              <div class="box-of-count-nafar-boxes">
               <span class="text-count-nafar">
                1 مسافر
               </span>
               <span class="fas fa-caret-down down-count-nafar">
               </span>
              </div>
              <div class="cbox-count-nafar">
               <div class="col-xs-12 cbox-count-nafar-ch bozorg-num">
                <div class="row">
                 <div class="col-xs-12 col-sm-6 col-6">
                  <div class="type-of-count-nafar">
                   <h6>
                    بزرگسال
                   </h6>
                   (بزرگتر از ۱۲ سال)
                  </div>
                 </div>
                 <div class="col-xs-12 col-sm-6 col-6">
                  <div class="num-of-count-nafar">
                   <i class="counting-of-count-nafar plus-nafar">
                    <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
                     <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
                     <path d="M432 256C432 269.3 421.3 280 408 280h-160v160c0 13.25-10.75 24.01-24 24.01S200 453.3 200 440v-160h-160c-13.25 0-24-10.74-24-23.99C16 242.8 26.75 232 40 232h160v-160c0-13.25 10.75-23.99 24-23.99S248 58.75 248 72v160h160C421.3 232 432 242.8 432 256z">
                     </path>
                    </svg>
                   </i>
                   <i class="number-count counting-of-count-nafar" data-min="1" data-number="1" data-value="l-bozorgsal" id="bozorgsal_prtal">
                    1
                   </i>
                   <i class="counting-of-count-nafar minus-nafar">
                    <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
                     <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
                     <path d="M432 256C432 269.3 421.3 280 408 280H40c-13.25 0-24-10.74-24-23.99C16 242.8 26.75 232 40 232h368C421.3 232 432 242.8 432 256z">
                     </path>
                    </svg>
                   </i>
                  </div>
                 </div>
                </div>
               </div>
               <div class="col-xs-12 cbox-count-nafar-ch koodak-num">
                <div class="row">
                 <div class="col-xs-12 col-sm-6 col-6">
                  <div class="type-of-count-nafar">
                   <h6>
                    کودک
                   </h6>
                   (بین
                                                                    2 الی 12 سال)
                  </div>
                 </div>
                 <div class="col-xs-12 col-sm-6 col-6">
                  <div class="num-of-count-nafar">
                   <i class="counting-of-count-nafar plus-nafar">
                    <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
                     <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
                     <path d="M432 256C432 269.3 421.3 280 408 280h-160v160c0 13.25-10.75 24.01-24 24.01S200 453.3 200 440v-160h-160c-13.25 0-24-10.74-24-23.99C16 242.8 26.75 232 40 232h160v-160c0-13.25 10.75-23.99 24-23.99S248 58.75 248 72v160h160C421.3 232 432 242.8 432 256z">
                     </path>
                    </svg>
                   </i>
                   <i class="number-count counting-of-count-nafar" data-min="0" data-number="0" data-value="l-kodak" id="kodak_prtal">
                    0
                   </i>
                   <i class="counting-of-count-nafar minus-nafar">
                    <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
                     <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
                     <path d="M432 256C432 269.3 421.3 280 408 280H40c-13.25 0-24-10.74-24-23.99C16 242.8 26.75 232 40 232h368C421.3 232 432 242.8 432 256z">
                     </path>
                    </svg>
                   </i>
                  </div>
                 </div>
                </div>
               </div>
               <div class="col-xs-12 cbox-count-nafar-ch nozad-num">
                <div class="row">
                 <div class="col-xs-12 col-sm-6 col-6">
                  <div class="type-of-count-nafar">
                   <h6>
                    نوزاد
                   </h6>
                   (کوچکتر از 2 سال)
                  </div>
                 </div>
                 <div class="col-xs-12 col-sm-6 col-6">
                  <div class="num-of-count-nafar">
                   <i class="counting-of-count-nafar plus-nafar">
                    <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
                     <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
                     <path d="M432 256C432 269.3 421.3 280 408 280h-160v160c0 13.25-10.75 24.01-24 24.01S200 453.3 200 440v-160h-160c-13.25 0-24-10.74-24-23.99C16 242.8 26.75 232 40 232h160v-160c0-13.25 10.75-23.99 24-23.99S248 58.75 248 72v160h160C421.3 232 432 242.8 432 256z">
                     </path>
                    </svg>
                   </i>
                   <i class="number-count counting-of-count-nafar" data-min="0" data-number="0" data-value="l-nozad" id="nozad_prtal">
                    0
                   </i>
                   <i class="counting-of-count-nafar minus-nafar">
                    <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
                     <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
                     <path d="M432 256C432 269.3 421.3 280 408 280H40c-13.25 0-24-10.74-24-23.99C16 242.8 26.75 232 40 232h368C421.3 232 432 242.8 432 256z">
                     </path>
                    </svg>
                   </i>
                  </div>
                 </div>
                </div>
               </div>
              </div>
             </div>
            </div>
            <div class="col-lg-2 col-md-6 col-sm-12 col-12 btn_s col_search">
             <button class="button_searchBox w-100" onclick="gds_ticket_portal_check()" type="button">
              <span>
               جستجو
              </span>
             </button>
            </div>
           </form>
          </div>
         </div>
        </div>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="i_modular_blog blog my-5 py-5">
    <div class="container">
     <div class="blog-grid">
      {if $articles[0] }
      <div class="__i_modular_c_item_class_0 div1">
       <a href="{$articles[0]['link']}">
        <img alt="{$articles[0]['title']}" class="__image_class__" src="{$articles[0]['image']}"/>
        <div>
         <div>
          <h4 class="__title_class__">
           {$articles[0]['title']}
          </h4>
          <span class="__heading_class__">
           {$articles[0]['heading']}
          </span>
         </div>
         <i>
          <svg viewbox="0 0 320 512" xmlns="http://www.w3.org/2000/svg">
           <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
           <path d="M315.3 411.3C312.2 414.4 308.1 416 304 416s-8.188-1.562-11.31-4.688L32 150.6v184.6c0 8.844-7.156 16-16 16s-16-7.156-16-16V111.9c0-8.812 7.141-15.97 15.95-16c0 0 224-.6562 224-.6562c8.812 0 15.97 7.125 16 15.97c.0313 8.812-7.109 16-15.95 16.03l-185.6 .5547l260.9 260.9C321.6 394.9 321.6 405.1 315.3 411.3z">
           </path>
          </svg>
         </i>
        </div>
       </a>
      </div>
      {/if}
      {if $articles[1] }
      <div class="__i_modular_c_item_class_1 div2">
       <a href="{$articles[1]['link']}">
        <img alt="{$articles[1]['title']}" class="__image_class__" src="{$articles[1]['image']}"/>
        <div>
         <div>
          <h4 class="__title_class__">
           {$articles[1]['title']}
          </h4>
          <span class="__heading_class__">
           {$articles[1]['heading']}
          </span>
         </div>
         <i>
          <svg viewbox="0 0 320 512" xmlns="http://www.w3.org/2000/svg">
           <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
           <path d="M315.3 411.3C312.2 414.4 308.1 416 304 416s-8.188-1.562-11.31-4.688L32 150.6v184.6c0 8.844-7.156 16-16 16s-16-7.156-16-16V111.9c0-8.812 7.141-15.97 15.95-16c0 0 224-.6562 224-.6562c8.812 0 15.97 7.125 16 15.97c.0313 8.812-7.109 16-15.95 16.03l-185.6 .5547l260.9 260.9C321.6 394.9 321.6 405.1 315.3 411.3z">
           </path>
          </svg>
         </i>
        </div>
       </a>
      </div>
      {/if}
      {if $articles[2] }
      <div class="__i_modular_c_item_class_2 div3">
       <a href="{$articles[2]['link']}">
        <img alt="{$articles[2]['title']}" class="__image_class__" src="{$articles[2]['image']}"/>
        <div>
         <div>
          <h4 class="__title_class__">
           {$articles[2]['title']}
          </h4>
          <span class="__heading_class__">
           {$articles[2]['heading']}
          </span>
         </div>
         <i>
          <svg viewbox="0 0 320 512" xmlns="http://www.w3.org/2000/svg">
           <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
           <path d="M315.3 411.3C312.2 414.4 308.1 416 304 416s-8.188-1.562-11.31-4.688L32 150.6v184.6c0 8.844-7.156 16-16 16s-16-7.156-16-16V111.9c0-8.812 7.141-15.97 15.95-16c0 0 224-.6562 224-.6562c8.812 0 15.97 7.125 16 15.97c.0313 8.812-7.109 16-15.95 16.03l-185.6 .5547l260.9 260.9C321.6 394.9 321.6 405.1 315.3 411.3z">
           </path>
          </svg>
         </i>
        </div>
       </a>
      </div>
      {/if}
      {if $articles[3] }
      <div class="__i_modular_c_item_class_3 div4">
       <a href="{$articles[3]['link']}">
        <img alt="{$articles[3]['title']}" class="__image_class__" src="{$articles[3]['image']}"/>
        <div>
         <div>
          <h4 class="__title_class__">
           {$articles[3]['title']}
          </h4>
          <span class="__heading_class__">
           {$articles[3]['heading']}
          </span>
         </div>
         <i>
          <svg viewbox="0 0 320 512" xmlns="http://www.w3.org/2000/svg">
           <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
           <path d="M315.3 411.3C312.2 414.4 308.1 416 304 416s-8.188-1.562-11.31-4.688L32 150.6v184.6c0 8.844-7.156 16-16 16s-16-7.156-16-16V111.9c0-8.812 7.141-15.97 15.95-16c0 0 224-.6562 224-.6562c8.812 0 15.97 7.125 16 15.97c.0313 8.812-7.109 16-15.95 16.03l-185.6 .5547l260.9 260.9C321.6 394.9 321.6 405.1 315.3 411.3z">
           </path>
          </svg>
         </i>
        </div>
       </a>
      </div>
      {/if}
     </div>
    </div>
   </section>
   <section class="i_modular_news news my-5 py-5">
    <div class="container">
     <h2 class="news_title">
      آخرین اخبار و رویدادها
     </h2>
     <div class="news-grid">
      <div class="div1">
       <h2>
        آخرین اخبار و رویدادها
       </h2>
       <p class="mt-auto">
        به روزترین و جذاب‌ترین اخبار از دنیای موارد مختلف در وب سایت ما!
       </p>
       <p>
        به عنوان یک منبع اطلاعاتی جامع، با تیم ما در اینجا همراه شوید تا از آخرین اخبار و رویدادهای جذاب جهان مطلع شوید. از آخرین پیشرفت‌های علمی و تکنولوژی تا بهترین روش‌ها برای بهبود سلامتی، از جدیدترین موضوعات محیط‌زیستی تا هنر و فرهنگ هر کشور... همه چیز در اینجا وجود دارد!
       </p>
      </div>
      {if $othe_itmes[0] }
      <div class="__i_modular_c_item_class_0 div2">
       <a href="{$othe_itmes[0]['link']}">
        <img alt="{$othe_itmes[0]['alt']}" class="__image_class__" src="{$othe_itmes[0]['image']}"/>
        <div>
         <div>
          <h4 class="__heading_class__">
           {$othe_itmes[0]['heading']}
          </h4>
          <span>
           بیشتر بدانید
           <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
            <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
            <path d="M447.1 256c0 13.25-10.76 24.01-24.01 24.01H83.9l132.7 126.6c9.625 9.156 9.969 24.41 .8125 33.94c-9.156 9.594-24.34 9.938-33.94 .8125l-176-168C2.695 268.9 .0078 262.6 .0078 256S2.695 243.2 7.445 238.6l176-168C193 61.51 208.2 61.85 217.4 71.45c9.156 9.5 8.812 24.75-.8125 33.94l-132.7 126.6h340.1C437.2 232 447.1 242.8 447.1 256z">
            </path>
           </svg>
          </span>
         </div>
        </div>
       </a>
      </div>
      {/if}
      {if $othe_itmes[1] }
      <div class="__i_modular_c_item_class_1 div3">
       <a class="news_box" href="{$othe_itmes[1]['link']}">
        <div class="news_box_img">
         <img alt="{$othe_itmes[1]['alt']}" class="__image_class__" src="{$othe_itmes[1]['image']}"/>
        </div>
        <div class="news_box_text">
         <h4 class="__heading_class__">
          {$othe_itmes[1]['heading']}
         </h4>
         <p class="__description__">
          همایش بیمارستان‌ها و مراکز درمانی با محوریت گردشگری سلامت با حضور نمایندگانی از ۳۰ کشور اسلامی و مدیران مراکز درمانی ایران و نمایندگان مجلس و وزیر میراث فرهنگی گردشگری و صنایع‌دستی، به مدت ۳ روز از ۲۵ تا ۲۷ تیرماه در سالن همایش‌های صدا و سیما برگزار می شود.
         </p>
         <span>
          بیشتر بدانید
          <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
           <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
           <path d="M447.1 256c0 13.25-10.76 24.01-24.01 24.01H83.9l132.7 126.6c9.625 9.156 9.969 24.41 .8125 33.94c-9.156 9.594-24.34 9.938-33.94 .8125l-176-168C2.695 268.9 .0078 262.6 .0078 256S2.695 243.2 7.445 238.6l176-168C193 61.51 208.2 61.85 217.4 71.45c9.156 9.5 8.812 24.75-.8125 33.94l-132.7 126.6h340.1C437.2 232 447.1 242.8 447.1 256z">
           </path>
          </svg>
         </span>
        </div>
       </a>
      </div>
      {/if}
      {if $othe_itmes[2] }
      <div class="__i_modular_c_item_class_2 div4">
       <a class="news_box" href="{$othe_itmes[2]['link']}">
        <div class="news_box_img">
         <img alt="{$othe_itmes[2]['alt']}" class="__image_class__" src="{$othe_itmes[2]['image']}"/>
        </div>
        <div class="news_box_text">
         <h4 class="__heading_class__">
          {$othe_itmes[2]['heading']}
         </h4>
         <p class="__description__">
          یکی از شرکت‌های معروف و معتبر در صنعت ریلی کشور، شرکت ریل سیر کوثر است که با افزایش کیفیت خدمات و امکانات و با ارائه قطارهای متنوع در کلاس‌های مختلف، توانسته باعث رشد ناوگان حمل‌و‌نقل ریلی کشور شود. این شرکت ۱۰۰درصد به‌شکل خصوصی اداره می‌شود و با همکاری شرکت‌ واگن‌ پارس، به تازگی از قطار پنج ستاره خود پرده‌برداری کرده است. قطاری که امکانات آن دست‌ کمی از پروازهای بیزینس کلاس ندارد و با فراهم کردن آسایش مسافران، شکل جدیدی از خدمات رسانی در شبکه ریلی کشور را نشان می‌دهد.
         </p>
         <span>
          بیشتر بدانید
          <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
           <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
           <path d="M447.1 256c0 13.25-10.76 24.01-24.01 24.01H83.9l132.7 126.6c9.625 9.156 9.969 24.41 .8125 33.94c-9.156 9.594-24.34 9.938-33.94 .8125l-176-168C2.695 268.9 .0078 262.6 .0078 256S2.695 243.2 7.445 238.6l176-168C193 61.51 208.2 61.85 217.4 71.45c9.156 9.5 8.812 24.75-.8125 33.94l-132.7 126.6h340.1C437.2 232 447.1 242.8 447.1 256z">
           </path>
          </svg>
         </span>
        </div>
       </a>
      </div>
      {/if}
      {if $othe_itmes[3] }
      <div class="__i_modular_c_item_class_3 div5">
       <a class="news_box" href="{$othe_itmes[3]['link']}">
        <div class="news_box_img">
         <img alt="{$othe_itmes[3]['alt']}" class="__image_class__" src="{$othe_itmes[3]['image']}"/>
        </div>
        <div class="news_box_text">
         <h4 class="__heading_class__">
          {$othe_itmes[3]['heading']}
         </h4>
         <p class="__description__">
          به نقل از خبرگزاری فرارو، در روزهای ۶ تا ۱۴ تیرماه برای نمایندگان صنعت گردشگری ایران، سفری با هدف آشنایی با مقاصد توریستی کشور روسیه برگزار شد. در این سفر که از طرف کمیته گردشگری شهر مسکو با همکاری کمیته دولتی گردشگری جمهوری تاتارستان برنامه‌ریزی شده بود، گفت‌وگوهای تجاری بین نمایندگان ایران، کازان و مسکو انجام شد و نمایندگان شهرهای مختلف گزارشی از زیرساخت‌ها و ظرفیت‌های توریستی خود برای گردشگران ایرانی، ارائه دادند. نمایندگان مسکو درباره نظیر جشن‌‌ها و فستیوال‌های فصلی، گردشگری غذایی و سرآشپزهای مشهور، پارک‌های متعدد،‌ تفریحات مختلف و سفرهای رودخانه‌ای و… صحبت کردند.
         </p>
         <span>
          بیشتر بدانید
          <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
           <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
           <path d="M447.1 256c0 13.25-10.76 24.01-24.01 24.01H83.9l132.7 126.6c9.625 9.156 9.969 24.41 .8125 33.94c-9.156 9.594-24.34 9.938-33.94 .8125l-176-168C2.695 268.9 .0078 262.6 .0078 256S2.695 243.2 7.445 238.6l176-168C193 61.51 208.2 61.85 217.4 71.45c9.156 9.5 8.812 24.75-.8125 33.94l-132.7 126.6h340.1C437.2 232 447.1 242.8 447.1 256z">
           </path>
          </svg>
         </span>
        </div>
       </a>
      </div>
      {/if}
     </div>
    </div>
   </section>
   <section class="i_modular_newsletter news_main mt-5 pt-5">
    <div class="container">
     <div class="news_text col-12">
      <h2 class="titel text-white">
       عضویت در خبرنامه
      </h2>
     </div>
     <form class="col-12 p-0">
      <label class="p-1 col-12 col-md-6 col-lg-3">
       <span>
        نام و نام خانوادگی :
       </span>
       <input class="full-name-js" id="NameSms" name="NameSms" placeholder="نام" type="text"/>
      </label>
      <label class="p-1 col-12 col-md-6 col-lg-3">
       <span>
        پست الکترونیکی :
       </span>
       <input class="email-js" id="EmailSms" name="EmailSms" placeholder="ایمیل" type="email"/>
      </label>
      <label class="p-1 col-12 col-md-6 col-lg-3">
       <span>
        شماره موبایل :
       </span>
       <input class="mobile-js" id="CellSms" name="CellSms" placeholder="موبایل" type="text"/>
      </label>
      <div class="p-1 col-6 col-lg-3 mb-2-5 mt-auto">
       <a class="__submit_class__ button" href="javascript:" onclick="submitNewsLetter()">
        ثبت
       </a>
      </div>
     </form>
    </div>
   </section>
  </main>
  <footer class="i_modular_footer footer">
   <div class="footer_main container">
    <ul class="m-0 p-0 d-flex">
     <li class="col-12 col-md-6 my-4 col-lg-4 call">
      <h6 href="{$smarty.const.ROOT_ADDRESS}/contactUs">
       تماس با ما
      </h6>
      <span>
       <i class="far fa-map-marker">
       </i>
       <span class="__address_class__">
        آدرس :  {$smarty.const.CLIENT_ADDRESS}
       </span>
      </span>
      <span>
       <i class="far fa-phone">
       </i>
       شماره :
       <a class="__mobile_class__" href="tel:{$smarty.const.CLIENT_MOBILE}">
        {$smarty.const.CLIENT_MOBILE}
       </a>
      </span>
      {assign var="socialLinks"  value=$about['social_links']|json_decode:true}
                                {assign var="socialLinksArray" value=['telegram'=&gt;'telegramHref','whatsapp'=&gt; 'whatsappHref','instagram' =&gt; 'instagramHref']}
    
                                {foreach $socialLinks as $key =&gt; $val}
                                        {assign var=$socialLinksArray[$val['social_media']] value=$val['link']}
                                {/foreach}
      <div class="__social_class__ footer_icons">
       <a class="__instagram_class__ fab fa-instagram footer_instagram" href="{if $telegramHref}{$instagramHref}{/if}">
        <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
         <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
         <path d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z">
         </path>
        </svg>
       </a>
       <a class="__whatsapp_class__ fab fa-whatsapp footer_whatsapp" href="{if $telegramHref}{$whatsappHref}{/if}">
        <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
         <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
         <path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z">
         </path>
        </svg>
       </a>
       <a class="__telegram_class__ fab fa-telegram footer_telegram" href="{if $telegramHref}{$telegramHref}{/if}">
        <svg viewbox="0 0 496 512" xmlns="http://www.w3.org/2000/svg">
         <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
         <path d="M248,8C111.033,8,0,119.033,0,256S111.033,504,248,504,496,392.967,496,256,384.967,8,248,8ZM362.952,176.66c-3.732,39.215-19.881,134.378-28.1,178.3-3.476,18.584-10.322,24.816-16.948,25.425-14.4,1.326-25.338-9.517-39.287-18.661-21.827-14.308-34.158-23.215-55.346-37.177-24.485-16.135-8.612-25,5.342-39.5,3.652-3.793,67.107-61.51,68.335-66.746.153-.655.3-3.1-1.154-4.384s-3.59-.849-5.135-.5q-3.283.746-104.608,69.142-14.845,10.194-26.894,9.934c-8.855-.191-25.888-5.006-38.551-9.123-15.531-5.048-27.875-7.717-26.8-16.291q.84-6.7,18.45-13.7,108.446-47.248,144.628-62.3c68.872-28.647,83.183-33.623,92.511-33.789,2.052-.034,6.639.474,9.61,2.885a10.452,10.452,0,0,1,3.53,6.716A43.765,43.765,0,0,1,362.952,176.66Z">
         </path>
        </svg>
       </a>
      </div>
      <div class="namads">
       <a href="javascript:">
        <img alt="Enamad1" src="project_files/images/certificate1.png"/>
       </a>
       <a href="javascript:">
        <img alt="namad-1" src="project_files/images/certificate5.png"/>
       </a>
       <a href="javascript:">
        <img alt="namad-2" src="project_files/images/certificate3.png"/>
       </a>
       <a href="javascript:">
        <img alt="namad-3" src="project_files/images/certificate4.png"/>
       </a>
      </div>
     </li>
     <li class="d-none d-md-block col-12 col-md-6 my-4 col-lg-4">
      <h6>
       دسترسی آسان
      </h6>
      <div class="asan">
       <a class="asan_link" href="{$smarty.const.ROOT_ADDRESS}/page/flight">
        پرواز
       </a>
       <a class="asan_link" href="{$smarty.const.ROOT_ADDRESS}/mag">
        وبلاگ
       </a>
       <a class="asan_link" href="{$smarty.const.ROOT_ADDRESS}/news">
        اخبار سایت
       </a>
       <a class="asan_link" href="{$smarty.const.ROOT_ADDRESS}/aboutIran">
        معرفی ايران
       </a>
       <a class="asan_link" href="{$smarty.const.ROOT_ADDRESS}/rules">
        قوانین و مقررات
       </a>
       <a class="asan_link" href="{$smarty.const.ROOT_ADDRESS}/aboutUs">
        درباره ما
       </a>
       <a class="asan_link" href="{$smarty.const.ROOT_ADDRESS}/contactUs">
        تماس با ما
       </a>
       <a class="asan_link" href="javascript:">
        فایل های راهنما
       </a>
      </div>
     </li>
     <li class="d-none d-lg-flex col-12 col-md-6 my-4 col-lg-4 flex-column">
      <h6 href="{$smarty.const.ROOT_ADDRESS}/aboutUs">
       درباره ما
      </h6>
      <p class="__aboutUs__">
       شرکت خدمات مسافرتی و گردشگری  آوا پرواز ایرانیان ، با سابقه ارزشمند در راستای ایرانگردی و جهانگردی و با بهره گیری از کادری مجرب آمادگی خویش را جهت ارائه خدمات ذیل اعلام میدارد. - صدور بلیت اتوبوس هواپیما و قطار بصورت حضوری و غیر حضوری - رزرو و معرفی اقامتگاه بومگردی هتل سوییت و ویلا - اجرای تورهای طبیعتگردی ایرانگردی جهانگردی
      </p>
      <a href="{$smarty.const.ROOT_ADDRESS}/aboutUs">
       بیشتر
       <svg viewbox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
        <!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
        <path d="M447.1 256c0 13.25-10.76 24.01-24.01 24.01H83.9l132.7 126.6c9.625 9.156 9.969 24.41 .8125 33.94c-9.156 9.594-24.34 9.938-33.94 .8125l-176-168C2.695 268.9 .0078 262.6 .0078 256S2.695 243.2 7.445 238.6l176-168C193 61.51 208.2 61.85 217.4 71.45c9.156 9.5 8.812 24.75-.8125 33.94l-132.7 126.6h340.1C437.2 232 447.1 242.8 447.1 256z">
        </path>
       </svg>
      </a>
     </li>
    </ul>
   </div>
   <div class="last_text col-12">
    <a class="last_a" href="https://www.iran-tech.com/" target="_blank">
     طراحی سایت گردشگری
    </a>
    <p class="last_p_text">
     : ایران تکنولوژی
    </p>
   </div>
  </footer>
 </body>
 <script src="project_files/js/jquery-3.4.1.min.js">
 </script>
 <script src="project_files/js/bootstrap.bundle.min.js">
 </script>
 <script src="project_files/js/owl.carousel.min.js">
 </script>
 <script src="project_files/js/mega-menu.js">
 </script>
 <script src="project_files/js/select2.min.js">
 </script>
 <script src="project_files/js/script.js">
 </script>
</html>
